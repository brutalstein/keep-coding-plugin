...
export interface RuntimeEvent { name: string; cwd: string; prompt?: string; stopGuardActive?: boolean }
export interface RuntimeDirective { continue: boolean; context?: string; blockReason?: string }
...
export class PollingCliAdapter implements ContinuityAdapter {
  ...
  async translate(_event: RuntimeEvent, service: KeepCodingService | null): Promise<RuntimeDirective> {
