...
describe("context payload benchmark", () => {
  it("reports a concrete reduction for unchanged and decision-only delivery", () => {
    ...
    try {
      ...
      store.savePlan(
        {
          goal: "Deliver a representative multi-phase production system.", nonGoals: [],
          ...
          doneWhen: ["all tests pass", "compiled artifact executes"], budget: { maxTokens: 50_000 }
        },
        Array.from({ length: 12 }, (_, index) => ({
          id: `phase-${index}`, title: `Phase ${index}`, goal: `Implement subsystem ${index}`,
          dependencies: index === 0 ? [] : [`phase-${index - 1}`], allowedScope: [`src/subsystem-${index}/**`],
          acceptanceCommands: [`npm run test -- subsystem-${index}`], maxAttempts: 3
        }))
      );
      for (let index = 0; index < 250; index += 1) {
        store.upsertGraphNode({
          id: `symbol:src/subsystem-0/file-${index}.ts:function:symbol${index}`,
          type: "symbol", label: `symbol${index}`, path: `src/subsystem-0/file-${index}.ts`, symbol: `symbol${index}`,
      ...
      store.recordDecision({ phaseId: "phase-0", title: "Use delta context", rationale: "Avoid retransmitting unchanged durable sections.", alternatives: ["Send the full snapshot every time"] });
      ...
      expect(unchanged).toMatchObject({ unchanged: true, changedSections: [] });
